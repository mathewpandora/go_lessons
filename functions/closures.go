...
//Нужно написать пример базщовго замякания и прокоментировать очень просто
...
func adder(n int) func(x int) int {
    sum := n
    return func(x int) int { //вернули функцию 
        sum += x //теперь в этой функции будет храниться sum и она не пропадет 
    return sum}
}